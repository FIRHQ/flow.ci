# flow.ci 中文文档

* [快速入门](./1. flow.ci 快速入门.md)

* [创建项目](./2. 创建项目.md)
  - [创建 GitHub 项目](./2.1 创建 GitHub 项目.md)
  - [创建 Bitbucket 项目](./2.2 创建 Bitbucket 项目.md)
  - [创建 GitLab 项目](./2.3 创建 GitLab 项目.md)

* [构建 Build](./3. 构建 Build.md)
  - [手动 Build](./3.1 手动 Build.md)
  - [Git 的 push 请求触发 Build](./3.2 git 的 push 请求触发 Build.md)
  - [Pull Request 触发 Build](./3.3 Pull Request 触发 Build.md)
  - [每日构建](./3.4 每日构建.md)

* [支持语言](./4. 支持语言.md)
  - [PHP](./4.1 PHP.md)
  - [Java for Android](./4.2 Java for Android.md)	
  - [Node.js](./4.3 Nodejs.md)
  - [Ruby](./4.4 Ruby.md)
  - [Python](./4.5 Python.md)

* [flow 设置](./5. flow 设置.md)
  - [插件的添加，修改，删除](./5.1 插件的添加，修改，删除.md)
  - [插件的 Inputs & Outputs](./5.2 插件的 Inputs & Outputs.md)
  - [插件列表](./5.3 插件列表.md)

* [FAQ](./6. FAQ.md)
  - [如何找到 Provisioning Profiles & 证书文件?](./6.1如何找到%20Provisioning%20Profiles%20%26%20证书文件%3F.md)

* [服务协议](./flow.ci 服务协议.md)

* [隐私协议](./flow.ci 隐私政策.md)
